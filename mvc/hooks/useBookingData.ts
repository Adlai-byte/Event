...
function mapBooking(b: any): Booking {
    ...
    try {
      ...
      if (dateRangeInfo.startDate && dateRangeInfo.endDate) {
        ...
        if (isConsecutive && allDates.length > 1) {
          ...
          formattedDate = allDates.map((dateStr: string) =>
            new Date(dateStr).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })
          ).join(' and ');
        } else if (allDates.length === 1) {
          formattedDate = new Date(allDates[0]).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });
        } else {
          formattedDate = `${startDate.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })} - ${endDate.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}`;
        }
      }
  ...
  if (!formattedDate) {
    formattedDate = eventDate.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });
  }

  const startTime = b.b_start_time ? new Date(`2000-01-01T${b.b_start_time}`).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true }) : '';
  const endTime = b.b_end_time ? new Date(`2000-01-01T${b.b_end_time}`).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true }) : '';
