...
export const AnalyticsView: React.FC<AdminAnalyticsProps> = ({ user, onNavigate, onLogout }) => {
  ...
  const StatCard = ({
    ...
    <View style={[styles.statCard, glowColor && { shadowColor: glowColor }]}>
      ...
      {change && (
        <Text
          style={[styles.statChange, trend === 'up' ? styles.statChangeUp : styles.statChangeDown]}
        >
          {trend === 'up' ? '↑' : '↓'} {change}
        </Text>
      )}
              ...
              <View style={styles.growthContainer}>
                ...
                <View style={styles.growthItem}>
                  ...
                  <Text style={styles.growthValue}>
                    {analytics.userGrowth.activeUsers.toLocaleString()}
                  </Text>
                </View>
                <View style={styles.growthItem}>
...
const createStyles = (isMobile: boolean, screenWidth: number) =>
  StyleSheet.create({
    ...
    statCard: {
      ...
      width: isMobile ? (screenWidth - 48) / 2 : (screenWidth - 300) / 3,
      elevation: 8,
