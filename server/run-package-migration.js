// Run this script to create package system tables
// Usage: node run-package-migration.js
...
async function runMigration() {
    ...
    const tableNames = ['service_package', 'package_category', 'package_item', 'booking_package'];
    ...
    for (let i = 0; i < tables.length; i++) {
        try {
            ...
            if (error.code === 'ER_TABLE_EXISTS_ERROR') {
                console.log(`- Table already exists: ${tableNames[i]}`);
            } else {
                console.error(`✗ Error creating ${tableNames[i]}:`, error.message);
            }
        }
    }
    ...
    // Verify tables exist
    console.log('\nVerifying tables...');
    const [rows] = await connection.execute("SHOW TABLES LIKE '%package%'");
    console.log('Package tables found:', rows.map(r => Object.values(r)[0]));
